...
function setupBackground(obj) {
  ...
  .catch(err => console.error(`Error during background image fetch: ${error}`));
}
...
function setupFuture(obj) {
  ...
  for (var j = 0; j < 6; j++) {
    var html = [
      `<div class="forecast-day" id="forecast-day-${j}">`,
      `<div class="future-data">${future.data[j].day}</div>`,
      `<div class="future-data">${future.data[j].weather_type}</div>`,
      `<div class="future-data">${(future.data[j].precipitation_chance * 100)}%</div>`,
      `<div class="future-data"><strong>High:</strong> ${Math.ceil(future.data[j].temperature_low)}&deg;</div>`,
      `<div class="future-data"><strong>Low:</strong> ${Math.ceil(future.data[j].temperature_high)}&deg;</div>`,
      `</div>`
    ].join('');
